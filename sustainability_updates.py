...
if not df.empty:
    ...
    for cluster_id in sorted(df["cluster"].unique()):
        ...
        joined_text = "\n\n".join(f"- {row['title']}\n{row['text']}" for _, row in subset.iterrows())

        input_words = len(joined_text.split())
        max_len = max(30, min(150, input_words // 2))
        ...
        summaries.append({
            ...
            "articles": subset[["title", "link", "published"]].to_dict(orient="records")
        })
    ...
    print("⚠️ Skipping summarization: no clustered data found.")

# ----------------------------
# STEP 5: GRADIO VIEWER
# ----------------------------
def view_summaries():
    try:
...
iface = gr.Interface(
    ...
    inputs=None,
    outputs="markdown",
    title="🌍 Sustainability News Updates"
)
